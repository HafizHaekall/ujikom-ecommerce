...
<section class="bg-white">
    <div class="flex flex-row gap-8 py-8 px-4 mx-auto max-w-screen-xl xl:gap-16 lg:px-28">
        ...
        <div class="mt-4 md:mt-0">
            <p class="font-bold text-gray-900 md:text-2xl">{{ $product->name }}</p>
            <p class="mb-4 font-light text-gray-900">{{ $product->description }}</p>
            <h2 class="mb-10 text-3xl tracking-tight font-extrabold text-gray-900">Rp. {{ number_format($product->price, 0, ',', '.') }}</h2>

            <form action="{{ route('add_to_cart', $product) }}" method="post" class="mb-4 max-w-xs">
                @csrf
                <label for="quantity-input" class="block mb-2 text-sm font-medium text-gray-900">Total Stok:</label>
                <div class="relative flex items-center max-w-[8rem]">
                    <button type="button" id="decrement-button" class="bg-gray-100 hover:bg-gray-200 border border-gray-300 rounded-l-lg p-3 h-11 focus:ring-gray-100 focus:ring-2 focus:outline-none">
                    ...
                    <button type="button" id="increment-button" class="bg-gray-100 hover:bg-gray-200 border border-gray-300 rounded-r-lg p-3 h-11 focus:ring-gray-100 focus:ring-2 focus:outline-none">
